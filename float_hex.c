...
static int	apply_flags(int exp, t_flags *f, char *inte, char *frac)
{
	if (f->pound == 1 || ft_strlen(frac) > 0 || f->prec > 0)
		strappend(&inte, ".");
	charappend(&frac, '0', f->prec);
	strappend(&inte, frac);
	free(frac);
	(exp < 0) ? strappend(&inte, "p-") : strappend(&inte, "p+");
	frac = ft_itoa_base(exp < 0 ? -exp : exp, 10);
	strappend(&inte, frac);
	free(frac);
	if (f->zero == 1 && f->minus == 0)
		charprepend(&inte, '0', f->minwid - 3);
	if (f->zero && !(f->minus) && f->sign && !(f->plus) && !(f->space))
		charprepend(&inte, '0', f->minwid - 2);
	strprepend(&inte, "0x");
	(f->sign == -1) ? strprepend(&inte, "-") : 0;
	if (f->sign == 1 && (f->plus == 1 || f->space == 1))
		(f->plus == 1) ? strprepend(&inte, "+") : strprepend(&inte, " ");
	(f->minus == 1) ? charappend(&inte, ' ', f->minwid) : 0;
